...
public class SubArrayEqualTarget {
	...
	public static boolean isSubArrayEqualTargetII(int[] arr, int target) {
		...
		for(int i = 0; i < arr.length; i++) {
			...
			else if(set.contains(curSum - target))	return true;
			set.add(curSum);
		}
		return false;
	}
	...
	public static boolean search(int[] sum, int left, int right, int target) {
		...
		if(sum[mid] > target)	return search(sum, left, mid-1, target);
		else if(sum[mid] < target)	return search(sum, mid+1, right, target);
		else	return true;
	}
