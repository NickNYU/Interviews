...
public class MatrixAreaOf1 {
	...
	public static void matrixAreaHelper(int[][] matrix, int row, int col, boolean[][] visited, int[] result, int sample) {
		if(row < 0 || row > matrix.length-1)	return;
		if(col < 0 || col > matrix[0].length-1)	return;
		if(visited[row][col])	return;
		visited[row][col] = true;
		if(matrix[row][col] != sample)	return;
		result[0] ++;
	...
	public static int matrixAreaII(int[][] matrix, int x, int y) {
		...
		while(!queue.isEmpty()) {
			...
			if(matrix[point.x][point.y] != matrix[x][y]) 
				continue;
			result++;
			if(row > 0)	queue.offer(new Point(row-1, col));
			if(row < matrix.length-1)	queue.offer(new Point(row+1, col));
			if(col > 0)	queue.offer(new Point(row, col-1));
			if(col < matrix[0].length-1)	queue.offer(new Point(row, col+1));
		}
		return result;
	}
